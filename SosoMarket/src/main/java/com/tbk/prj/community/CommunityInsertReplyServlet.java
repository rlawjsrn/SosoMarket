	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (memberId != null && !memberId.isEmpty()) {
			if (commentDetail != null && !commentDetail.isEmpty()) {
				...
				out.println("<html>");
				out.println("<head>");
				out.println("<title>Comment Text Empty Error</title>");
				out.println("</head>");
				out.println("<body>");

				out.println("<script type='text/javascript'>");
				out.println("alert('댓글 내용을 입력해주세요 ! ');");
				out.println("window.location.href='/SosoMarket/CommunityPostDetail.do?postId=" + postId + "';"); // Redirect to the
																										// product page
				out.println("</script>");
